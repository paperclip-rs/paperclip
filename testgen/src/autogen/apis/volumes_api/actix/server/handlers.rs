...
pub fn configure<T: server::Volumes + 'static>(cfg: &mut ServiceConfig) {
    cfg
       ...
       .service(
            actix_web::web::resource("/volumes/{volume_id}/rebuild-history")
                .name("get_rebuild_history")
                .guard(actix_web::guard::Get())
                .route(actix_web::web::get().to(get_rebuild_history::<T, A>))
       )
       ...
       .service(
            actix_web::web::resource("/volumes/{volume_id}/property")
                .name("put_volume_property")
                .guard(actix_web::guard::Put())
                .route(actix_web::web::put().to(put_volume_property::<T, A>))
       )
       ...
       .service(
            actix_web::web::resource("/volumes/{volume_id}/size")
                .name("put_volume_size")
                .guard(actix_web::guard::Put())
                .route(actix_web::web::put().to(put_volume_size::<T, A>))
       )
       ...
       .service(
            actix_web::web::resource("/snapshots/{snapshot_id}/volumes/{volume_id}")
                .name("put_snapshot_volume")
                .guard(actix_web::guard::Put())
                .route(actix_web::web::put().to(put_snapshot_volume::<T, A>))
       );
...
async fn get_rebuild_history<T: server::Volumes + 'static, A: FromRequest + 'static>(_token: A, path: Path<uuid::Uuid>) -> Result<Json<serde_json::Value>, RestError<crate::models::RestJsonError>> {
    T::get_rebuild_history(crate::apis::actix_server::Path(path.into_inner())).await.map(Json)
}
...
async fn put_volume_property<T: server::Volumes + 'static, A: FromRequest + 'static>(_token: A, path: Path<uuid::Uuid>) -> Result<Json<crate::models::Volume>, RestError<crate::models::RestJsonError>> {
    T::put_volume_property(crate::apis::actix_server::Path(path.into_inner())).await.map(Json)
}
...
async fn put_volume_size<T: server::Volumes + 'static, A: FromRequest + 'static>(_token: A, path: Path<uuid::Uuid>) -> Result<Json<crate::models::Volume>, RestError<crate::models::RestJsonError>> {
    T::put_volume_size(crate::apis::actix_server::Path(path.into_inner())).await.map(Json)
}
...
async fn put_snapshot_volume<T: server::Volumes + 'static, A: FromRequest + 'static>(_token: A, path: Path<uuid::Uuid, uuid::Uuid>) -> Result<Json<crate::models::Volume>, RestError<crate::models::RestJsonError>> {
    T::put_snapshot_volume(crate::apis::actix_server::Path(path.into_inner())).await.map(Json)
}
