...
impl PropertyDataType {
    ...
    fn set_disc_model(&mut self, parent: &str, name: &str) {
        self.set_if_unresolved(Self::DiscModel(parent.to_string(), name.to_string()));
    }
...
//pub(crate) type RcVec<T> = Rc<Vec<T>>;
...
pub(crate) struct Property {
    ...
    /// Inline models discovered through the schema of this very model.s
...
impl Property {
    ...
    fn with_kind(
        ...
        match schema_kind {
            ...
            openapiv3::SchemaKind::AllOf { all_of } => {
                ...
                let model = Self::from(&schema.schema_data).with_one_all_of(first_model);
                model
            }
            ...
            // why do we get AnySchema, simply because we have a default?
            openapiv3::SchemaKind::Any(any_schema) => match &any_schema.typ {
                Some(typ) => match typ.as_str() {
                    ...
                    _ => Property::default().with_data_type_any(false),
                },
                None => Property::default().with_data_type_any(false),
            },
        }
    }
    ...
    fn post_process_dt(data_type: &mut PropertyDataType, is_decl: bool) {
        match data_type.clone() {
            ...
            PropertyDataType::Number(_) => data_type.resolve_format("u64", "u64"),
