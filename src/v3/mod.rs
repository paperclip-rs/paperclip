...
impl OpenApiV3 {
    ...
    fn ensure_template(&self, templates: &Vec<&GenTemplateFile>) -> Result<(), std::io::Error> {
        templates
            .iter()
            .map(|template| self.ensure_template_path(template.target_prefix(), true))
            .collect::<Result<(), std::io::Error>>()?;
        templates
            .iter()
            .map(|template| self.ensure_template_path(template.target_prefix(), false))
            .collect::<Result<(), std::io::Error>>()
    }
    ...
    fn render_models(&self, models: &Vec<Property>) -> Result<(), std::io::Error> {
        for property in models {
            ...
            for template in &self.model_templates {
                let mustache = std::fs::read_to_string(template.tpl_path())?;
